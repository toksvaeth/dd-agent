...
class _TemplateCache(object):
    ...
    def get_templates(self, identifier):
        ...
        # hit
        if identifier in self.kv_templates:
        ...
        if identifier in self.auto_conf_templates:
            ...
            templates[CONFIG_FROM_AUTOCONF] = auto_conf_tpls
...
class AbstractConfigStore(object):
    ...
    def read_config_from_store(self, identifier):
        ...
        try:
            ...
            if not res:
                log.debug("Could not find templates for identifier {} in "
                          "the config store, trying with auto-config...".format(identifier))
